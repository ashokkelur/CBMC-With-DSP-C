  ...
  const c_qualifierst &qualifiers)
{
  ...
  else if(src.id()==ID_fixedbv)
  {
    ...
    if(width==config.ansi_c.short_fixed_width)
         if(to_fixedbv_type(src).get_integer_bits() == 1)
             return q+"short __fixed";
         else
             return q+"float";
    if(width==config.ansi_c.fixed_width)
      return q+"__fixed";
    if(width==config.ansi_c.short_accum_width)
      return q+"short __accum";
    if(width==config.ansi_c.accum_width)
      return q+"__accum";
  }
  ...
  else if(type.id()==ID_fixedbv)
  {
    ...
    if(dest!="" && isdigit(dest[dest.size()-1]))
    {
      if(src.type()==float_type())
        ...
        dest+="l";
      else if(src.type()==fixed_type())
        dest+="r";
      else if(src.type()==accum_type())
        dest+="a";
    }
  }
