...
/*** DCP-C Specific tokens */

%token TOK_X_MEM_LABEL
%token TOK_Y_MEM_LABEL
%token TOK_CM_MEM_LABEL
%token TOK_CPM_CONST_MEM_LABEL
%token TOK_CODE_MEM_LABEL

%token TOK_SATURATION
%token TOK_FIXED
%token TOK_ACCUM

%token TOK_FIXED_CONST
%token TOK_ACCUM_CONST
...
dsp_c_mem_label:
          TOK_X_MEM_LABEL          { $$=$1; set($$, ID_x_mem_label); } /* { init($$); }  */
        | TOK_Y_MEM_LABEL          { $$=$1; set($$, ID_y_mem_label); }
        | TOK_CM_MEM_LABEL         { $$=$1; set($$, ID_cm_mem_label); }
        | TOK_CPM_CONST_MEM_LABEL  { $$=$1; set($$, ID_cpm_mem_label); }
        | TOK_CODE_MEM_LABEL       { $$=$1; set($$, ID_code_mem_label); }
        ;

dsp_c_type_name:
          TOK_SATURATION   { $$=$1; set($$, ID_sat); }   /* TODO: Get new basic type */
        | TOK_FIXED        { $$=$1; set($$, ID_fixed); }
        | TOK_ACCUM        { $$=$1; set($$, ID_accum); }
