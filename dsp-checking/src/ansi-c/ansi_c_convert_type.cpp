...
void ansi_c_convert_typet::read_rec(const typet &type)
{
  if(type.id() == ID_fixed)
      fixed_cnt++;
  else if(type.id() == ID_accum)
      accum_cnt++;
  else if((type.id() == ID_x_mem_label) || (type.id() == ID_y_mem_label) ||
         (type.id() == ID_cm_mem_label) || (type.id() == ID_cpm_mem_label) ||
         (type.id() == ID_code_mem_label))
  {
       /* TODO: Handle memory labels */
  }
...
void ansi_c_convert_typet::write(typet &type)
{
  ...
  else if(fixed_cnt)
  {
     if(short_cnt)
     {
         if(unsigned_cnt)
         {
             std::cout<<"Can't handle unsigned fixed yet!\n";
         }

         type = fixed_short_type();
     }
     else
     {
         if(unsigned_cnt)
  ...
  else if(accum_cnt)
  {
     if(short_cnt)
     {
         if(unsigned_cnt)
         {
             std::cout<<"Can't handle unsigned accum yet!\n";
         }
         type = accum_short_type();
     }
     else
     {
         if(unsigned_cnt)
