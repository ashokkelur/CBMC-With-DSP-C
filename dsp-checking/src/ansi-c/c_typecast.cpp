  ...
  const typet &type)
{
  ...
  {
    if(width<=config.ansi_c.short_fixed_width)
      return SHORT_FIXED;
    else if(width<=config.ansi_c.fixed_width)
      return FIXED;
    else if(width<=config.ansi_c.short_accum_width)
      return SHORT_ACCUM;
    else if(width<=config.ansi_c.accum_width)
      return ACCUM;
  ...
  c_typet c_type)
{
  ...
  case SHORT_FIXED: new_type=fixed_short_type(); break;
  case FIXED:       new_type=fixed_type(); break;
  case SHORT_ACCUM: new_type=accum_short_type(); break;
  case ACCUM:       new_type=accum_type(); break;
  default: return;
  }
